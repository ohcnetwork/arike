...
feature 'Create Patient spec' do
  ...
  #   end
  # end
  context "Personal Details of Patient", js: true do
    scenario "Edit Name in Personal Details" do
      @id = Patient.last.id
      @user = FactoryBot.create(:user, role: User.roles[:superuser], verified: true)
      login_as(@user)
      ...
      within(find("tbody.patients_table")) do
        click find_by_id(@id)
      end

      click_on('Edit Personal Details')

      fill_in "patient[full_name]", with: "Sample Test"
      click_on "Submit"
      expect(page).to have_text("Sample Test")
    end
  end
end
