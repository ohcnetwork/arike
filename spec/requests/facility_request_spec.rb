...
RSpec.describe "Facility as superuser", type: :request do
  ...
  it "create a valid CHC" do
    ...
    post facilities_path, params: { facility: { kind: kind, name: name, state: state, district: district, address: address, lsg_body_id: lsg_body_id, ward_id: ward_id, phone: phone, pincode: pincode } }
  ...
  it "create an invalid PHC (without parent_id)" do
    ...
    phone = Faker::Number.number(digits: 10)
    lsg_body_id = LsgBody.first.id
    post facilities_path, params: { facility: { kind: kind, name: name, state: state, district: district, address: address, lsg_body_id: lsg_body_id, phone: phone, pincode: pincode } }
