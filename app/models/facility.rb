class Facility < ApplicationRecord
  ...
  has_many :primary_nurses,
           -> { where(role: User.roles[:primary_nurse]) },
           class_name: 'User',
           foreign_key: 'facility_id',
           inverse_of: :facility
  has_many :secondary_nurses,
           -> { where(role: User.roles[:secondary_nurse]) },
           class_name: 'User',
           foreign_key: 'facility_id',
           inverse_of: :facility
  belongs_to :lsg_body_info,
             class_name: 'LsgBody',
             foreign_key: 'lsg_body_id',
             inverse_of: :facilities
  belongs_to :ward_info,
             class_name: 'Ward',
             foreign_key: 'ward_id',
             inverse_of: :facilities
  belongs_to :state
  ...
  enum kinds: { primary: 'PHC', secondary: 'CHC', hospital: 'Hospital' }

  validates :parent_id,
            presence: true,
            if: -> { kind == Facility.kinds[:primary] }
  validates :parent_id,
            absence: true,
            if: -> { kind == Facility.kinds[:secondary] }
