...
ActiveRecord::Schema.define(version: 2021_04_08_170209) do
  # These are extensions that must be enabled in order to support this database
  enable_extension 'pgcrypto'
  enable_extension 'plpgsql'

  create_table 'diseases',
               id: :uuid,
  ...
  create_table 'lsg_bodies',
    ...
    t.string 'code'
    t.uuid 'districts_id'
    t.index ['districts_id'], name: 'index_lsg_bodies_on_districts_id'
  end

  create_table 'patient_disease_summaries',
               id: :uuid,
               default: -> { 'gen_random_uuid()' },
               force: :cascade do |t|
    t.uuid 'patient_id'
  ...
  create_table 'users',
    ...
    t.index %w[email phone],
            name: 'index_users_on_email_and_phone',
            unique: true
    t.index ['facility_id'], name: 'index_users_on_facility_id'
  end

  create_table 'visit_details',
               id: :uuid,
               default: -> { 'gen_random_uuid()' },
               force: :cascade do |t|
    t.integer 'akps'
  ...
  add_foreign_key 'districts', 'states', column: 'states_id'
  add_foreign_key 'facilities', 'facilities', column: 'parent_id'
  add_foreign_key 'lsg_bodies', 'districts', column: 'districts_id'
